...
public interface ICouponRespository extends CrudRepository<Coupon, Long> {
	...
	@Query("select new com.baruch.coupons.datapresentation.CouponBasicData(c.title, c.company.name, c.endDate, c.price, c.id) from Coupon c")
	public List<ICouponDataObject> getAllCoupons();

	@Query("select new com.baruch.coupons.datapresentation.CouponBasicData(c.title, c.company.name, c.endDate, c.price, c.id) from Coupon c where c.startDate <= ?1 and c.endDate > ?1")
	public List<ICouponDataObject> getAllCouponsForCustomer(Date now);

	@Query("select new com.baruch.coupons.datapresentation.CouponFullDataForAdmin(c.amount, c.id, c.company.id, c.price, c.title, c.description, c.image, c.company.name, c.category, c.startDate, c.endDate) from Coupon c where c.id = ?1")
	public ICouponDataObject getCouponForAdmin(long couponID);
	
	@Query("select new com.baruch.coupons.datapresentation.CouponFullDataDefault(c.amount, c.id, c.price, c.title, c.description, c.image, c.company.name, c.category, c.startDate, c.endDate) from Coupon c where c.id = ?1")
	public ICouponDataObject getCouponDefault(long couponID);

	@Query("select new com.baruch.coupons.datapresentation.CouponBasicData(c.title, c.company.name, c.endDate, c.price, c.id) from Coupon c where c in (select p.coupon from Purchase p where p.user.id = ?1) and c.price <= ?2")
	public List<ICouponDataObject> getPurchasedCouponsByMaxPrice(long userID, float price);
	
	@Query("select new com.baruch.coupons.datapresentation.CouponBasicData(c.title, c.company.name, c.endDate, c.price, c.id) from Coupon c where c.company.id = ?1")
	public List<ICouponDataObject> getCouponsByCompany(long companyID);
	
	@Query("select new com.baruch.coupons.datapresentation.CouponBasicData(c.title, c.company.name, c.endDate, c.price, c.id) from Coupon c where c.category= ?1")

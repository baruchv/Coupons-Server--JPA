...
public interface IPurchasesRepository extends CrudRepository<Purchase, Long> {
	...
	"(p.id, p.coupon.amount, p.timeStamp, p.coupon.price, p.coupon.title) from Purchase p where p.user.id = ?1")
	public List<IPurchaseDataObject> getAllPurchasesForCustomer(long userID);

	@Query("select new com.baruch.coupons.datapresentation.purchase.PurchaseDataForCustomer(p.coupon.amount, p.timeStamp, p.coupon.price, p.coupon.title, p.coupon.company.name) from Purchase p where p.user.id = ?1")
	public IPurchaseDataObject getPurchaseForCustomer(long purchaseID);

	@Query("select new com.baruch.coupons.datapresentation.purchase.PurchaseBasicData"
			+ "(p.id, p.coupon.amount, p.timeStamp, p.coupon.price, p.coupon.title) from Purchase p where p.coupon.company.id = ?1")
	...
	@Query("select new com.baruch.coupons.datapresentation.purchase.PurchaseBasicData" + 
	"(p.id, p.coupon.amount, p.timeStamp, p.coupon.price, p.coupon.title) from Purchase p")
